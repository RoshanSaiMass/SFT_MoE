...
block generalized to a noisy-top-k MoE):
  ...
  1. SNIP layer selection   -- one backward pass on a calibration batch
                                ranks all transformer blocks by task
                                saliency; the lowest-saliency block(s)
                                are picked for substitution (Eq. 8-9).
  2. Calibration + init      -- forward hooks capture the ORIGINAL block's
                                input/output on ~64 calibration images;
                                every expert in the MoE filter block is
                                initialized from the pseudo-inverse
                                solution (Eq. 3-4), so the filter starts
                                out reproducing the original block.
  3. Component substitution  -- vit.blocks[idx] <- MoEFilterBlock(idx);
                                everything except the filter block(s),
                                LayerNorm affine params, and the head is
                                frozen; fine-tune only those.

Usage (smoke test, no VTAB download needed):
    python train_sfp_moe.py --smoketest --epochs 10

Usage (real VTAB-1k task):
...
def collect_block_io(model, layer_indices, calib_batches, device, n_images):
    ...
    pseudo-inverse initialization (paper Eq. 3-4)."""
...
def main():
    ...
    # 1. SNIP: rank blocks by task saliency, replace the least salient one(s)
    ...
    print(f"[SFP] SNIP scores (lower = more replaceable): "
          f"{dict(sorted(scores.items(), key=lambda kv: kv[1]))}")
    print(f"[SFP] Replacing transformer block(s) {layer_indices} "
          f"out of {len(scores)} total blocks")

    # 2. calibration IO on the ORIGINAL blocks, for pseudo-inverse init
    ...
    # 3. component substitution: swap in MoE filter blocks
    moe_kwargs = dict(
    ...
    # 4. freeze everything except filter block(s) + LayerNorm + head
